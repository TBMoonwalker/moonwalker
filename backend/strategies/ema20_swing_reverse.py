...
EMA20_STATE_VERSION_SUFFIX = ":v3"


class Strategy:
    """EMA20 swing reverse strategy implementation.

    The strategy only evaluates EMA20. It returns True when the latest closed
    candle sits below its EMA20 and that closed candle's EMA20 value is lower
    ...
    def _evaluate_trigger_candidate(
        ...
        *,
        ...
        """Return the bearish trigger flag plus the candidate closed-candle state."""
        ...
        return trigger_down, close_value_float, ema20_value_float
    ...
    def _find_latest_qualified_state_from_series(
        ...
        for idx in range(1, len(close_series) - 1):
            ...
            if trigger_down and close_value is not None and ema20_value is not None:
    ...
    async def _load_persisted_state(
        ...
        try:
            ...
            logging.error(
                "Cannot load persisted EMA20 swing reverse state for %s on %s: %s",
                symbol,
                self.timeframe,
                exc,
                exc_info=True,
    ...
    async def _persist_previous_state(
        ...
        try:
            await run_sqlite_write_with_retry(
                _persist,
                (
                    "persisting ema20 swing reverse state "
            ...
            logging.error(
                "Cannot persist EMA20 swing reverse state for %s on %s: %s",
                symbol,
                self.timeframe,
                exc,
                exc_info=True,
    ...
    async def run(self, symbol: str, type: str) -> bool:
        """Evaluate EMA20 bearish progression for a symbol."""
        del type
        result = False
        ...
        try:
            ...
            logging.error(
                "Cannot run EMA20 swing reverse strategy for %s, check indicators.log: %s",
                symbol,
                exc,
                exc_info=True,
            )
