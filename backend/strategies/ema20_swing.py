...
class Strategy:
    ...
    where the line turns from falling to rising and returns True when both the
    latest EMA20 value and that swing-low value are higher than the previously
    ...
    def _evaluate_swing_candidate(
        ...
        *,
        ...
        """Return the EMA20 up-swing flag plus the candidate swing state."""
        ...
        return swing_up, ema20_prev_value, ema20_now_value
    ...
    def _find_latest_qualified_swing_state_from_series(
        ...
        for idx in range(2, len(ema20_series)):
            swing_up, swing_value, ema20_value = cls._evaluate_swing_candidate(
                ema20_now=ema20_series.iloc[idx],
                ema20_prev=ema20_series.iloc[idx - 1],
                ema20_prev2=ema20_series.iloc[idx - 2],
            )
            if swing_up and swing_value is not None and ema20_value is not None:
    ...
    async def _persist_previous_state(
        ...
        async def _persist() -> None:
            await model.Ema20SwingState.update_or_create(
                defaults={
                    "previous_swing_low": swing_value,
    ...
    async def _remember_previous_state(
        ...
        self._previous_state_by_symbol[symbol] = (swing_value, ema20_value)
        await self._persist_previous_state(symbol, swing_value, ema20_value)
    ...
    async def run(self, symbol: str, type: str) -> bool:
        """Evaluate EMA20 swing progression for a symbol."""
        del type
        result = False
        ...
        try:
            ...
            ema20_series = self._build_ema20_series(close_series)
            swing_up, current_swing_value, current_ema20 = (
            ...
            logging_json = {
                ...
                "ema20(prev1)": ema20_series.iloc[-2],
                "ema20_swing(current)": (current_swing_value if swing_up else None),
                "ema20_swing(previous)": previous_swing_value,
                "ema20(current)": current_ema20,
                "ema20(previous)": previous_ema20,
                "swing_up_detected": swing_up,
