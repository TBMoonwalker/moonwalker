...
def test_ema20_swing_history_scan_returns_latest_qualified_swing_state() -> None:
    ema20_series = pd.Series([12.0, 12.0, 9.9, 9.7, 9.8, 10.2, 10.0, 10.3])

    latest_state = Strategy._find_latest_qualified_swing_state_from_series(ema20_series)

    assert latest_state == (10.0, 10.3)


def test_ema20_swing_candidate_rejects_missing_numbers() -> None:
    swing_up, swing_value, ema20_value = Strategy._evaluate_swing_candidate(
        ema20_now=10.2,
...
@pytest.mark.asyncio
async def test_ema20_swing_first_swing_only_primes_state(monkeypatch) -> None:
    ...
    strategy.indicators = _StubIndicators(
        ema20_values=[9.8],
        close_series_values=[_ema20_series(10.0, 9.7, 9.8)],
    )
...
@pytest.mark.asyncio
async def test_ema20_swing_returns_true_when_ema20_and_swing_rise(
    monkeypatch,
) -> None:
    monkeypatch.setattr(Strategy, "_load_persisted_state", _no_op_load)
    monkeypatch.setattr(Strategy, "_persist_previous_state", _no_op_persist)
    ...
    strategy = Strategy("4h")
    strategy.indicators = _StubIndicators(
        ema20_values=[9.8, 10.3],
        close_series_values=[
            _ema20_series(10.0, 9.7, 9.8),
            _ema20_series(10.2, 10.0, 10.3),
        ],
    )
...
async def test_ema20_swing_bootstraps_missing_state_without_trading(
    ...
    strategy.indicators = _StubIndicators(
        ema20_values=[10.3],
        close_series_values=[_ema20_series(10.2, 10.0, 10.3)],
    )

    result = await strategy.run("ERA/USDC", "buy")

    assert result is False
    assert strategy._previous_state_by_symbol["ERA/USDC"] == (10.0, 10.3)
    assert persisted_store[("ERA/USDC", "4h")] == (10.0, 10.3)
...
async def test_ema20_swing_persist_previous_state_swallows_write_errors(
    ...
    await strategy._persist_previous_state("ERA/USDC", 10.0, 10.3)
...
@pytest.mark.asyncio
async def test_ema20_swing_restart_uses_persisted_state_for_next_higher_swing(
    monkeypatch,
) -> None:
    persisted_store = _install_fake_state_store(monkeypatch)
    monkeypatch.setattr(
    ...
    assert await first_strategy.run("ERA/USDC", "buy") is False
    assert persisted_store[("ERA/USDC", "4h")] == (9.7, 9.8)
