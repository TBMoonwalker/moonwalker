...
class Autopilot:
    ...
    async def calculate_trading_settings(self, funds_locked, config):
        trading_settings = {}
        if config.get("autopilot", False):
            ...
            if threshold_percent >= float(config.get("autopilot_high_threshold", False)):
                ...
                trading_settings["sl_timeout"] = int(config.get(
                    "autopilot_high_sl_timeout", 0
                ))
